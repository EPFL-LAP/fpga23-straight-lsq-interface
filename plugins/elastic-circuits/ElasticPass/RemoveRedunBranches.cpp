			...
			while(j < enode_dag_cpy->size()) {
				...
				if(enode_2->type == branch_type && enode_2->BB == enode_1->BB && enode_2->producer_to_branch == producer && enode_2 != enode_1
					...
					if(enode_1_branch_cond == enode_2_branch_cond && enode_1->is_negated_input->at(1) == enode_2->is_negated_input->at(1)) {
						...
						// Erase this redundant branch from the enode_dag_cpy to not loop over it again
						...
						if(enode_1_branch_cond == enode_2_branch_cond && enode_1->is_negated_input->at(1) != enode_2->is_negated_input->at(1)) {
							...
							switch(network_flag) {
								...
								case constCntrl:
								case memDeps:
									assert(enode_2->false_branch_succs_Ctrl->size() == 1 || enode_2->false_branch_succs_Ctrl_lsq->size() == 1);
									if(enode_2->false_branch_succs_Ctrl->size() == 1) {
		...
		case memDeps:
			if(constCntrl_flag) {
				if(redun_br->true_branch_succs_Ctrl->size() > 0) {
					...
					// 30/10/2022: Because of the logic of the smart LSQ, I might run into a situation where I need to merge two Branches one in the JustCntrl network and the other in the CntrlOrder network into 1 Branch
						// the goal is to do this by being consistent with the network of the branch_to_keep
					if(br_to_keep->true_branch_succs_Ctrl->size() > 0 || br_to_keep->false_branch_succs_Ctrl->size() > 0 ) {
						...
						assert(br_to_keep->true_branch_succs_Ctrl_lsq->size() > 0 || br_to_keep->false_branch_succs_Ctrl_lsq->size() > 0 );

						br_to_keep->true_branch_succs_Ctrl_lsq->push_back(redun_br->true_branch_succs_Ctrl->at(0));

						// let this successor point to the branch_to_keep, instead of the redun_branch
						true_idx = returnPosRemoveNode(redun_br->true_branch_succs_Ctrl->at(0)->JustCntrlPreds, redun_br); 
				
						// erase from the JustCntrlPreds and push to the CntrlOrderPreds!
						// if the redun_br->true_branch_succs_Ctrl->at(0) is of type Phi_c, make sure to transfer the 2 preds to CntrlOrder!!
						for(int r = 0; r < redun_br->true_branch_succs_Ctrl->at(0)->JustCntrlPreds->size(); r++) {
							ENode* one_pred_of_redun_br_succ = redun_br->true_branch_succs_Ctrl->at(0)->JustCntrlPreds->at(r);
							redun_br->true_branch_succs_Ctrl->at(0)->CntrlOrderPreds->push_back(one_pred_of_redun_br_succ);

							if(one_pred_of_redun_br_succ->type != Branch_c) {
								// search for the redun_br->true_branch_succs_Ctrl->at(0) in the succs of redun_br->true_branch_succs_Ctrl->at(0)->JustCntrlPreds->at(r)
								auto pos_ = std::find(one_pred_of_redun_br_succ->JustCntrlSuccs->begin(), one_pred_of_redun_br_succ->JustCntrlSuccs->end(), redun_br->true_branch_succs_Ctrl->at(0));
								assert(pos_ != one_pred_of_redun_br_succ->JustCntrlSuccs->end());
								one_pred_of_redun_br_succ->CntrlOrderSuccs->push_back(redun_br->true_branch_succs_Ctrl->at(0));
								one_pred_of_redun_br_succ->JustCntrlSuccs->erase(pos_);
							}
							
						}
						redun_br->true_branch_succs_Ctrl->at(0)->CntrlOrderPreds->at(true_idx) = br_to_keep;
						redun_br->true_branch_succs_Ctrl->at(0)->JustCntrlPreds->clear(); 
				...
				if(redun_br->true_branch_succs_Ctrl_lsq->size() > 0) {
					...
					// the goal is to do this by being consistent with the network of the branch_to_keep
					if(br_to_keep->true_branch_succs_Ctrl->size() > 0 || br_to_keep->false_branch_succs_Ctrl->size() > 0) {
						br_to_keep->true_branch_succs_Ctrl->push_back(redun_br->true_branch_succs_Ctrl_lsq->at(0));

						// let this successor point to the branch_to_keep, instead of the redun_branch
						true_idx = returnPosRemoveNode(redun_br->true_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds, redun_br); 

						// erase from the CntrlOrderPreds and push to the JustCntrlPreds!
						// if the redun_br->true_branch_succs_Ctrl->at(0) is of type Phi_c, make sure to transfer the 2 preds to CntrlOrder!!
							// loop over the predecessors of the single successor of the Branch
						for(int r = 0; r < redun_br->true_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->size(); r++) {
							ENode* one_pred_of_redun_br_succ = redun_br->true_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->at(r);
							redun_br->true_branch_succs_Ctrl_lsq->at(0)->JustCntrlPreds->push_back(one_pred_of_redun_br_succ);
							
							// only consider non branch preds of my redun_branch_succs since branches are naturally handled by this function!
							if(one_pred_of_redun_br_succ->type != Branch_c) {
								// search for the redun_br->true_branch_succs_Ctrl_lsq->at(0) in the succs of redun_br->true_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->at(r)
								auto pos_ = std::find(one_pred_of_redun_br_succ->CntrlOrderSuccs->begin(), one_pred_of_redun_br_succ->CntrlOrderSuccs->end(), redun_br->true_branch_succs_Ctrl_lsq->at(0));
								assert(pos_ != one_pred_of_redun_br_succ->CntrlOrderSuccs->end());
								one_pred_of_redun_br_succ->JustCntrlSuccs->push_back(redun_br->true_branch_succs_Ctrl_lsq->at(0));
								one_pred_of_redun_br_succ->CntrlOrderSuccs->erase(pos_);
							}
						}
						redun_br->true_branch_succs_Ctrl_lsq->at(0)->JustCntrlPreds->at(true_idx) = br_to_keep;
						redun_br->true_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->clear(); 
			...
			if(constCntrl_flag) {
				if(redun_br->false_branch_succs_Ctrl->size() > 0) {
					...
					if(br_to_keep->true_branch_succs_Ctrl->size() > 0 || br_to_keep->false_branch_succs_Ctrl->size() > 0 ) {
						...
						br_to_keep->false_branch_succs_Ctrl_lsq->push_back(redun_br->false_branch_succs_Ctrl->at(0));

						// let this successor point to the branch_to_keep, instead of the redun_branch
						false_idx = returnPosRemoveNode(redun_br->false_branch_succs_Ctrl->at(0)->JustCntrlPreds, redun_br);

						// erase from the JustCntrlPreds and push to the CntrlOrderPreds!
						// if the redun_br->false_branch_succs_Ctrl->at(0) is of type Phi_c, make sure to transfer the 2 preds to CntrlOrder!!
						for(int r = 0; r < redun_br->false_branch_succs_Ctrl->at(0)->JustCntrlPreds->size(); r++) {
							ENode* one_pred_of_redun_br_succ = redun_br->false_branch_succs_Ctrl->at(0)->JustCntrlPreds->at(r);
							redun_br->false_branch_succs_Ctrl->at(0)->CntrlOrderPreds->push_back(one_pred_of_redun_br_succ);

							if(one_pred_of_redun_br_succ->type != Branch_c) {
								// search for the redun_br->false_branch_succs_Ctrl->at(0) in the succs of redun_br->false_branch_succs_Ctrl->at(0)->JustCntrlPreds->at(r)
								auto pos_ = std::find(one_pred_of_redun_br_succ->JustCntrlSuccs->begin(), one_pred_of_redun_br_succ->JustCntrlSuccs->end(), redun_br->false_branch_succs_Ctrl->at(0));
								assert(pos_ != one_pred_of_redun_br_succ->JustCntrlSuccs->end());
								one_pred_of_redun_br_succ->CntrlOrderSuccs->push_back(redun_br->false_branch_succs_Ctrl->at(0));
								one_pred_of_redun_br_succ->JustCntrlSuccs->erase(pos_);
							}
						}
						redun_br->false_branch_succs_Ctrl->at(0)->CntrlOrderPreds->at(false_idx) = br_to_keep;
						redun_br->false_branch_succs_Ctrl->at(0)->JustCntrlPreds->clear();  
				...
				if(redun_br->false_branch_succs_Ctrl_lsq->size() > 0) {
					...
					// the goal is to do this by being consistent with the network of the branch_to_keep
					if(br_to_keep->true_branch_succs_Ctrl->size() > 0 || br_to_keep->false_branch_succs_Ctrl->size() > 0) {
						br_to_keep->false_branch_succs_Ctrl->push_back(redun_br->false_branch_succs_Ctrl_lsq->at(0));
						...
						// erase from the CntrlOrderPreds and push to the JustCntrlPreds!
						// if the redun_br->false_branch_succs_Ctrl->at(0) is of type Phi_c, make sure to transfer the 2 preds to CntrlOrder!!
						for(int r = 0; r < redun_br->false_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->size(); r++) {
							ENode* one_pred_of_redun_br_succ = redun_br->false_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->at(r);
							redun_br->false_branch_succs_Ctrl_lsq->at(0)->JustCntrlPreds->push_back(one_pred_of_redun_br_succ);
							
							// only consider non branch preds of my redun_branch_succs since branches are naturally handled by this function!
							if(one_pred_of_redun_br_succ->type != Branch_c) {
								// search for the redun_br->false_branch_succs_Ctrl_lsq->at(0) in the succs of redun_br->false_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->at(r)
								auto pos_ = std::find(one_pred_of_redun_br_succ->CntrlOrderSuccs->begin(), one_pred_of_redun_br_succ->CntrlOrderSuccs->end(), redun_br->false_branch_succs_Ctrl_lsq->at(0));
								assert(pos_ != one_pred_of_redun_br_succ->CntrlOrderSuccs->end());
								one_pred_of_redun_br_succ->JustCntrlSuccs->push_back(redun_br->false_branch_succs_Ctrl_lsq->at(0));
								one_pred_of_redun_br_succ->CntrlOrderSuccs->erase(pos_);
							}
						}
						redun_br->false_branch_succs_Ctrl_lsq->at(0)->JustCntrlPreds->at(false_idx) = br_to_keep;
						redun_br->false_branch_succs_Ctrl_lsq->at(0)->CntrlOrderPreds->clear(); 
