...
// TAKE CARE: URGENT!!
// Aya: it is temporary and it only supports a loop-nest of depth two!!!!!!!!!!!!
void CircuitGenerator::TEMP_deleteExtraPhis() {
	...
	for(auto& enode: *enode_dag) {
		if(enode->type == Phi_n) {
			assert(!enode->is_non_loop_gsa_mux && !enode->is_mem_dep_component); // sanity check: I never have a Phi_n with any of these flags true!!
			...
			assert(enode->CntrlPreds->size() > 0 && enode->CntrlSuccs->size() > 0);
			assert(enode->CntrlPreds->at(0) != enode && enode->CntrlSuccs->at(0) != enode);

			if(enode->CntrlPreds->at(0)->type == Phi_n && enode->CntrlSuccs->at(0)->type != Phi_n) {  // identify the last Phi_n in the chain of phis (i.e., the one closest to the consumer!)
	...
	// second manage PHi_c
	/*for(auto& enode* : enode_dag) {
		if(enode->type == Phi_c && !enode->is_non_loop_gsa_mux && !enode->is_mem_dep_component)	{
