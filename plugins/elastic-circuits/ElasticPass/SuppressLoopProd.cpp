...
// AYA: new function added on 09/11/2022
void CircuitGenerator::addLoopsExits_ConsInProdOut(ENode* producer, ENode* consumer, std::vector<int>& modif_consBB_deps) {
	std::vector<Loop*> loops_in_nest;
 	FindLoopsInNest(BBMap->at(consumer->BB)->loop, loops_in_nest);
	...
	for(int i = 0; i < loops_in_nest.size(); i++) {
		...
		//  consider the loop only if the producer is not part of it!
		if(!loops_in_nest.at(i)->contains(producer->BB)) {
...
void CircuitGenerator::addMergeRegeneration(std::ofstream& dbg_file, bool const_trigger_network_flag) {
	...
	node_type phi_type; // specifies if we will create a Phi_c or Phi_n
	std::string phi_name; 
