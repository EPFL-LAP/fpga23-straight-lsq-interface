...
int getTanh (inout_int_t A[1000], in_int_t addr[1000]) {
	...
	hlsv_if_addr = fopen("../INPUT_VECTORS/input_addr.dat", "a");
	fprintf(hlsv_if_addr, "[[transaction]] %d\n", hlsv_transaction_id);
	for(hlsv_i0 = 0; hlsv_i0 < 1000; hlsv_i0++){
			fprintf(hlsv_if_addr, "0x%08llx\n", (long long)addr[hlsv_i0]);
		}
	fprintf(hlsv_if_addr, "[[/transaction]]\n");
	fclose(hlsv_if_addr);
